    ...
    while True:
        ...
        if amount.isdigit():
            ...
            if MIN_BET <= amount <= MAX_BET:
                ...
                print(f"Enter a value between ${MIN_BET} - {MAX_BET}")
        else:
            print("Please enter a number.")
    return amount
...
def spin(balance):
    ...
    print(f"You are betting {bet} on {lines} lines. The total bet amount is {total_bet}")

    slots = get_slot_machine_spin(ROWS, COLS, symbol_count)
    print_slot_machine(slots)

    winnings, winnings_lines = check_winnings(slots, lines, bet, symbol_value)
    print(f"You won ${winnings}! On {winnings_lines} number of lines.")
